...
class ElementPlacer {
  ...
  _createImageSticker(cfg) {
    ...
    // Click action support for sticker note panels.
    if (cfg.data.clickAction === 'openChaiPanel' || cfg.data.clickAction === 'openNotesPanel') {
      el.setAttribute('role', 'button');
      el.setAttribute('tabindex', '0');
      el.classList.add('image-sticker--note');
      const openNotes = (event) => {
        if (event) event.stopPropagation();
        const noteKey = cfg.data.noteKey || 'coffee';
        if (window.__creatorNotesPanel) {
          window.__creatorNotesPanel.open(noteKey);
        } else if (window.__chaiPanel) {
          window.__chaiPanel.open(noteKey);
        }
      };
      el.addEventListener('click', openNotes);
      el.addEventListener('keydown', (e) => {
  ...
  _createDraggableSticker(cfg) {
    ...
    const bubbleText = cfg.data.speechBubble;
    el.innerHTML = `
      ${bubbleText ? `<div class="speech-bubble" aria-hidden="true">${bubbleText}</div>` : ''}
      <img src="${cfg.data.src}" alt="${cfg.data.alt || ''}" width="${cfg.width}" height="${cfg.height}" loading="lazy" draggable="false">
    `;

    const opensNotes = cfg.data.clickAction === 'openNotesPanel' || cfg.data.clickAction === 'openChaiPanel';
    const openNotes = () => {
      const noteKey = cfg.data.noteKey || 'coffee';
      if (window.__creatorNotesPanel) {
        window.__creatorNotesPanel.open(noteKey);
    ...
    if (opensNotes) {
      el.classList.add('draggable-sticker--note');
      el.setAttribute('role', 'button');
      el.setAttribute('tabindex', '0');
      el.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
    ...
    const onMove = (e) => {
      ...
      if (Math.hypot(e.clientX - startX, e.clientY - startY) > CONFIG.clickThreshold) {
        moved = true;
      }
  ...
  _createInfoCard(cfg) {
    if (cfg.data.shortcut) {
      return this._createFolderShortcut(cfg);
    }
    ...
    if (variant === 'about') {
      const visualHtml = cfg.data.photo
        ? `<img src="${cfg.data.photo}" alt="${cfg.data.photoAlt || cfg.data.title}" class="info-card__cover" />`
        : `<div class="info-card__quote-mark">"</div>`;
      el.innerHTML = `
  ...
  _createWorkStack(cfg) {
    ...
    const isBrandStack = cfg.data.stackType === 'brands' && Array.isArray(cfg.data.previewNames);
    const dotColors = deckCards.slice(0, 4).map(c => c.color);

    const deckHtml = isBrandStack
      ? `
        <div class="work-stack__brand-preview" aria-hidden="true">
          ${cfg.data.previewNames.slice(0, 12).map(name => `<span>${name}</span>`).join('')}
        </div>
      `
      : deckCards.map((card, i) => {
        ...
        const thumbHtml = card.thumb
          ...
          : `<div class="work-stack__card-thumb work-stack__card-thumb--placeholder">${card.thumbText || ''}</div>`;
  ...
  // ── FOLDER SHORTCUT — desktop-style home interaction ──
  _createFolderShortcut(cfg) {
    const shortcut = cfg.data.shortcut;
    const el = document.createElement('article');
    el.className = 'canvas-element folder-shortcut';
    el.id = cfg.id;
    el.setAttribute('data-clickable', 'true');
    el.setAttribute('role', 'button');
    el.setAttribute('tabindex', '0');
    el.setAttribute('aria-label', `Open ${shortcut.label}`);

    el.innerHTML = `
      <img class="folder-shortcut__icon" src="${shortcut.folder}" alt="" loading="lazy" draggable="false">
      <span class="folder-shortcut__label">${shortcut.label}</span>
    `;

    const selectOnce = () => {
      el.classList.add('is-selected');
      window.clearTimeout(el._folderSelectTimer);
      el._folderSelectTimer = window.setTimeout(() => {
        el.classList.remove('is-selected');
      }, 260);
    };

    el.addEventListener('pointerdown', selectOnce);
    el.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        selectOnce();
        if (window.__cardTransitions) {
          window.__cardTransitions.openCard(cfg.id);
        } else {
          el.click();
        }
      }
